...
class LLM:
    ...
    def __init__(self, args):
        ...
        if args.openai_api:
            ...
            self.tokenizer = AutoTokenizer.from_pretrained(
                "gpt2", fast_tokenizer=False
            )  # TODO: For ChatGPT we should use a different one
    ...
    def generate(self, prompt, max_tokens, stop=None):
        ...
        if args.openai_api:
            use_chat_api = ("turbo" in args.model and not args.azure) or (
                "gpt-4" in args.model and args.azure
            )
            if use_chat_api:
...
def main():
    ...
    parser.add_argument(
        "--quick_test", type=int, default=None, help="Quickly test a few examples"
    )
    ...
    parser.add_argument(
        ...
        type=bool,
        default=False,
        help="Whether to remove the documents in the demos",
    )
    parser.add_argument(
        "--fewer_doc_in_demo",
        type=bool,
        default=False,
        help="Whether to use fewer documents in the demos",
    )
    parser.add_argument(
        "--ndoc_in_demo",
        type=int,
        default=None,
        help="When using --fewer_doc_in_demo, use this to designate how many docs in demo",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Whether to use summary data or extraction data for documents. Option: None, `summary`, `extraction`",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="The query to use in interactive mode, either `doc_id` (corresponding to interact in paper) or `search` (corresponding to inlinesearch in paper).",
    )
    parser.add_argument(
        "--retriever",
        type=str,
        default=None,
        help="When using interactive search mode, which retriever to use. Options: `tfidf`, `gtr-t5-large`",
    )
    parser.add_argument(
        "--retriever_device",
        type=str,
    ...
    parser.add_argument(
        "--max_turn", type=int, default=10, help="Max number of all actions"
    )
    parser.add_argument(
        "--max_doc_show",
        type=int,
        default=3,
        help="Max number of documents to show at one time.",
    )
    parser.add_argument(
        "--force_cite_show",
        type=bool,
        default=False,
        help="Force citing the documents that are shown to the model",
    )
    ...
    for idx, eval_item in enumerate(tqdm(eval_data)):
        ...
        doc_list = (
            get_shorter_text(eval_item, eval_item["docs"], args.ndoc, args.use_shorter)
            if args.use_shorter is not None
            else eval_item["docs"][: args.ndoc]
        )
    ...
    if (
        args.interactive
        and args.interactive_query == "search"
        and "gtr" in args.retriever
    ):
        ...
        for _ in range(args.num_samples):
            if args.interactive:
                ...
                while True:
                    ...
                    output = llm.generate(
                        new_prompt,
                        min(args.max_new_tokens, args.max_length - new_prompt_len),
                        stop=["\n", "\n\n"],
                    )
                    ...
                    if len(inline_doc) > 0:
                        output = (
                            "Output: " + output
                        )  # "Output: " was included in inline_doc
                    ...
                    if output.strip().lower()[:3] == "end":
                        ...
                        "sorry" in output.lower()
                        and (
                            "relevant document" in output.lower()
                            or "relevant information" in output.lower()
                        )
                        or "none of the documents" in output.lower()
                    ):
                        # Instruction-tuned model may abstain from answer the question
                        break
                    elif (
                        output.strip().lower()[:5] == "check"
                        or output.strip().lower()[:6] == "search"
                    ):
                        # Checkout or search documents
                        if args.interactive_query == "search":
                            query = (
                                output.replace("Search:", "")
                                .replace("search:", "")
                                .strip()
                            )
                            if len(doc_list) == 0:
                            ...
                            show_doc_ids = [
                                int(r[1:]) - 1 for r in re.findall(r"\[\d+", output)
                            ]  # In text citation id starts from 1
                            show_doc_ids = [
                                doc_id
                                for doc_id in show_doc_ids
                                if doc_id < len(doc_list) and doc_id >= 0
                            ]
                            show_doc_ids = show_doc_ids[
                                : args.max_doc_show
                            ]  # Avoiding showing too many documents
                        else:
                        ...
                        inline_doc = "".join(
                            [
                                make_doc_prompt(
                                    doc_list[doc_id], doc_id, prompt_data["doc_prompt"]
                                )
                                for doc_id in show_doc_ids
                            ]
                        )
